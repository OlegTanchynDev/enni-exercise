...
Doorkeeper.configure do
  ...
  # This block backs the browser /oauth/authorize flow. The take-home seeds an
  # access token directly (see db/seeds.rb), so this path isn't exercised in the
  # API tests, but we wire it to Devise so the flow works if used.
  resource_owner_authenticator do
    current_user || warden.authenticate!(scope: :user)
  end

  # Tokens carry resource-specific scopes; the API base controller authorises
  # them per child controller (e.g. :read, :bookings).
  default_scopes :read
  optional_scopes :bookings, :write
